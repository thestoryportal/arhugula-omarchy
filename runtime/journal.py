...
from .contracts import ContractError, Event, Interaction, VoiceObservation, decode, encode, loads
...
def _event(value):
    if not isinstance(value, (Event, Interaction, VoiceObservation)):
        raise JournalError("journal accepts only execution or observation records")
        ...
        with self.dispatch_lock:
            ...
            try:
                ...
                for sequence, identifier, payload in rows:
                    event = loads(payload)
                    if not isinstance(event, (Event, Interaction, VoiceObservation)) or event.event_id != identifier:
